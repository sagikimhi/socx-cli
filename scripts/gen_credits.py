...
def _set_license(metadata: PackageMetadata) -> None:
    license_field = metadata.get(
        "license-expression", metadata.get("license", "")
    )
    license_name = (
        license_field
...
def _get_deps(
    ...
    for dep_name, dep_req in base_deps.items():
        if dep_name not in metadata or dep_name == "socx-cli":
            continue
    ...
    while again:
        ...
        for pkg_name in metadata:
            if pkg_name in deps:
                for pkg_dependency in metadata[pkg_name].get(
                    "requires-dist", []
                ):
                    ...
                    if (
                        ...
                        and (
                            not extra_marker
                            or extra_marker in deps[pkg_name]["extras"]
                        )
                    ):
