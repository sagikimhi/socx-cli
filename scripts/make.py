...
def shell(
    ...
    subprocess.run(
        cmd, shell=True, check=True, stderr=subprocess.STDOUT, **kwargs
    )
    return None
...
def setup() -> None:
    ...
    if not shutil.which("uv"):
        err = (
            "make: setup: uv must be installed, see "
...
def run(version: str, cmd: str, *args: str, **kwargs: Any) -> None:
    ...
    try:
        if version == "default":
            ...
            with environ(
                UV_PROJECT_ENVIRONMENT=f".venvs/{version}", MULTIRUN="1"
            ):
                subprocess.run([cmd, *args], **kwargs)
...
def clean() -> None:
    ...
    for dirpath in Path(".").rglob("*/"):
        if (
            dirpath.parts[0] not in (".venv", ".venvs")
            and dirpath.name in cache_dirs
        ):
...
def main() -> int:
    ...
    while args:
        ...
        if cmd == "clean":
            ...
            multirun("duty", "check_api", "check_code", "check_docs")
            run("default", "duty", "check_api")
        elif cmd in {"check_code", "check_docs", "check_types", "test"}:
...
if __name__ == "__main__":
    try:
        ...
        if (code := process.returncode) == 139:
            cmd = " ".join(process.cmd)
            print(
                f"✗ (python{process.python_version})  '{cmd}'"
                " failed with return code {code} (segfault)",
                file=sys.stderr,
            )
