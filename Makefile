...
changelog: ## Update the project's CHANGELOG.md from the git commit log
	$(HIDE)$(UV) tool run git-changelog \
		--in-place \
		--parse-refs \
		--git-trailers \
		--omit-empty-versions \
		--style conventional \
		--input CHANGELOG.md \
		--config-file $(CWD)/config/git-changelog.toml \
...
export_svg: uv ## Export help menus of all socx commands as svg images
	...
	$(HIDE)$(UV) run termtosvg -m 100 -M 1000000 -t base16_default_dark -c "socx -NC --help" $(SVG_DIR)/socx-cli.svg
	...
	$(HIDE)$(UV) run termtosvg -m 100 -M 1000000 -t base16_default_dark -c "socx git -r $$HOME/.project/git/languages/systemverilog -i '*' -e 'uvm' -t 25 -p log" $(SVG_DIR)/socx-git-log.svg
	$(HIDE)$(UV) run termtosvg -m 100 -M 1000000 -t base16_default_dark -c "socx git -r $$HOME/.project/git/languages/systemverilog -i '*' -e 'uvm' -t 25 -p diff HEAD^ HEAD" $(SVG_DIR)/socx-git-diff.svg
	$(HIDE)$(UV) run termtosvg -m 100 -M 1000000 -t base16_default_dark -c "socx git -r $$HOME/.project/git/languages/systemverilog -i '*' -e 'uvm' -t 25 -p show --stat" $(SVG_DIR)/socx-git-show.svg
