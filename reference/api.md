...
- **`result`** (`TestResult`) – Return the result enum once a test has completed.
...
- **`pid`** (`int`) – Return the process identifier for the test's subprocess.
- **`process`** (`Process | None`) – Return a psutil.Process wrapper for the test's subprocess.
- **`ref`** (`int`) – Return the object's assigned unique identifier.
- **`result`** (`TestResult`) – Return the result enum once a test has completed.
...
NA: StrEnum Test has not yet finished running and therefore result is non-applicable.
